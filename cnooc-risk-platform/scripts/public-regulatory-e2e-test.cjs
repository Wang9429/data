#!/usr/bin/env node
/**
 * Phase 5 公共监管链路端到端测试（数据解析 + 浏览器导航）
 */
const fs = require('fs');
...
const chains = [
  {
    ...
    pages: ['global-legal-entities', 'global-regions', 'data-governance', 'warnings', 'rectification']
  },
  {
    ...
    pages: ['global-regions', 'cross-border-compliance', 'rectification']
  },
  {
    ...
    pages: ['data-governance', 'cross-domain-risks', 'global-legal-entities', 'rectification']
  },
  {
    name: '测试4：覆盖盲区→法人→数据源→数据质量→KRI→风险→整改',
    steps: [
    ...
    pages: ['coverage-gaps', 'global-legal-entities', 'data-governance', 'rectification']
  },
  {
    ...
    pages: ['platform-operations', 'data-governance', 'warnings', 'rectification']
  },
  {
    ...
    pages: ['cross-border-compliance', 'data-governance', 'warnings', 'rectification']
  },
  {
    ...
    pages: ['cross-domain-risks', 'data-governance', 'cross-border-compliance', 'rectification']
  }
];
...
async function browserNavTest() {
  ...
  await page.waitForFunction(() => typeof App !== 'undefined' && typeof App.navigatePublic === 'function', { timeout: 10000 });
  await page.evaluate(() => {
  ...
  const navTest = await page.evaluate(async () => {
    if (typeof App === 'undefined') return { ok: false, reason: 'App not loaded' };
    App.publicNavHistory = [];
    App.navigatePublic('global-legal-entities', { entityId: 'B001' });
    await new Promise(r => setTimeout(r, 200));
    const ctx1 = { ...(App.publicNavigationContext || {}) };
    ...
    return { ok: histLen >= 1 && ctxKept, histLen, afterBack, ctx1: ctx1.entityId };
  });

  await browser.close();
  server.close();
  return { browser: navTest.ok ? '通过' : '不通过', detail: navTest };
}
...
(async () => {
  ...
  const output = { dataChainTests: summary, browserNav: browserResult, allPass: summary.every(s => s.result === '通过') };
