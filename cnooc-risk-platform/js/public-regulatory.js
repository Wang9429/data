...
Object.assign(App, {
  ...
  renderWarningsDemoContextBanner(ctx) {
    if (!ctx) return '';
    const owner = ctx.objectLevel === '区域' ? '集团投资管理部' : (ctx.objectLevel === '法人' ? 'B公司投资管理部' : `${ctx.objectName}责任部门`);
    return `<div class="card" style="margin-bottom:16px;padding:14px 18px;border-left:4px solid #2563eb;background:#f8fbff">
      <div class="info-grid" style="grid-template-columns:repeat(2,minmax(0,1fr));gap:12px">
        <div class="info-item"><div class="info-label">当前监管对象</div><div class="info-value"><b>${ctx.objectName}</b></div></div>
        <div class="info-item"><div class="info-label">风险信号</div><div class="info-value">${ctx.riskSignal || '—'}</div></div>
        <div class="info-item"><div class="info-label">责任组织</div><div class="info-value">${owner}</div></div>
        <div class="info-item"><div class="info-label">下一步</div><div class="info-value">${this.renderPublicLinkButton('进入投资穿透分析', `App.openInvestmentPenetrationFromWarnings('${ctx.riskId || 'risk-2'}')`)}</div></div>
      </div>
    </div>`;
  },

  renderPenetrationObjectPathBar(riskId = 'risk-2') {
    ...
    return `<div class="card" style="margin-bottom:12px;padding:12px 16px">
      <div class="info-label">对象路径</div>
      <div class="info-value" style="margin-top:4px;font-size:14px">
        <a onclick="App.navigate('dashboard')">集团</a> / 中东区域 / 沙特阿拉伯 / ${w.unit || 'B公司'} / ${w.project || '某境外能源项目'} / 投后经营跟踪 / <b>${w.name || '风险事项'}</b>
      </div>
    </div>`;
  },

  renderPenetrationRiskSummary(riskId = 'risk-2') {
    const w = (APP_DATA.warnings || []).find(x => x.id === riskId) || (APP_DATA.warnings || [])[0] || {};
    ...
    return `<div class="card" style="margin-bottom:16px">
      <div class="info-grid" style="grid-template-columns:repeat(3,minmax(0,1fr))">
        <div class="info-item"><div class="info-label">风险事项</div><div class="info-value">${w.name || '—'}</div></div>
        <div class="info-item"><div class="info-label">KRI</div><div class="info-value">${w.indicator || '投资收益率'} · 3.2%</div></div>
        ...
        <div class="info-item"><div class="info-label">整改 / 验证</div><div class="info-value">${rect ? rect.status : '整改中'} · ${rect && rect.status === '已关闭' ? '已验证' : '待验证'}</div></div>
      </div>
      <p style="margin-top:10px">${this.renderPublicLinkButton('进入投资整改闭环', `App.goToRectificationWithDemoContext('${w.id || riskId}')`)} ${this.renderPublicLinkButton('返回集团风险态势', `App.traceUpToGroupRiskPerspective('${w.id || riskId}')`)}</p>
    </div>`;
  },

  renderPenetrationCurrentLevelStack(riskId = 'risk-2') {
    return this.renderPenetrationObjectPathBar(riskId) + this.renderPenetrationRiskSummary(riskId);
  },
  ...
  renderRectificationGroupSupervisionClosure(riskId = 'risk-2') {
    ...
    const rect = (APP_DATA.rectificationTasks || []).find(t => t.riskMatterId === w.id || t.riskId === w.id) || (APP_DATA.rectificationTasks || []).find(t => t.id === 'RECT-202601001');
    const owner = `${w.unit || 'B公司'}投资管理部 → 项目公司`;
    const overdue = rect && rect.deadline && rect.status !== '已关闭' ? '是' : '否';
    return `<div class="card" style="margin-bottom:16px">
      <div class="card-title">关联风险整改 · ${w.name || '—'}</div>
      <table class="data-table"><thead><tr><th>整改事项</th><th>责任组织</th><th>状态</th><th>进度</th><th>是否超期</th><th>证据</th><th>验证结果</th></tr></thead>
      <tbody><tr><td>${rect ? rect.title : '投资收益改善专项整改'}</td><td>${owner}</td><td>${rect ? rect.status : '整改中'}</td><td>${rect ? rect.progress : 60}%</td><td>${overdue}</td><td>${rect && rect.progress >= 60 ? '已提交' : '待补充'}</td><td>${rect && rect.status === '已关闭' ? '已通过' : '待验证'}</td></tr></tbody></table>
      <p style="margin-top:10px">${this.renderPublicLinkButton('返回集团风险态势', `App.traceUpToGroupRiskPerspective('${w.id || riskId}')`)} ${this.renderPublicLinkButton('返回穿透分析', `App.openInvestmentPenetrationFromWarnings('${w.id || riskId}')`)}</p>
    </div>`;
  },

  renderGroupPenetrationSupervisionChain() {
    return '';
  },
  ...
  renderGroupFocusObjectsTable() {
    ...
    const rows = [
      ['中东区域', '区域', '风险集中度偏高', '8项重大风险占集团42%', '区域风险指数', '红色预警', '集团投资管理部', '整改中', '待验证', mk('中东区域', '区域', '集团总部', '风险集中度偏高', '8项重大风险占集团42%', 'risk-2')],
      ['沙特阿拉伯', '国家', 'KRI连续异常', '收益率连续2期低于阈值', '投资收益率', '橙色预警', 'B公司投资管理部', '整改中', '待验证', mk('沙特阿拉伯', '国家', '中东区域', 'KRI连续异常', '收益率连续2期低于阈值', 'risk-2')],
      ['B公司', '法人', '多风险事项关联', '关联风险事项5项', '综合风险评分', '红色预警', 'B公司投资管理部', '整改中', '待验证', mk('B公司', '法人', '中东区域 / 沙特', '多风险事项关联', '关联风险事项5项 · 整改超期2项', 'risk-2')],
      ['某境外能源项目', '项目', '重大事项影响较大', '集团督办 · 收益偏离', '投资收益率', '红色预警', '项目公司', '整改中', '待验证', mk('某境外能源项目', '项目', 'B公司', '重大事项影响较大', '集团督办 · 收益偏离重大', 'risk-2')],
      ['投后经营跟踪', '投资事项', '流程跟踪异常', '投后跟踪频率不足', '利润下降率', '黄色预警', 'B公司境外事业部', '整改中', '待验证', mk('投后经营跟踪', '投资事项', '某境外能源项目', '流程跟踪异常', '投后跟踪频率不足', 'risk-2')],
      ['投资收益未达预期风险', '风险事项', 'KRI超阈值', '红色预警', '投资收益率 3.2%', '红色预警', 'B公司投资管理部 → 项目公司', '整改中', '待验证', `App.drillDownFromGroupPerspective({objectName:'投资收益未达预期风险',objectLevel:'风险事项',parentOrg:'投后经营跟踪',riskSignal:'KRI超阈值',whyFocus:'红色预警',riskId:'risk-2'},{showDetail:true})`]
    ];
    const headers = ['监管对象', '层级', '风险信号', '关注原因', 'KRI', '预警状态', '责任组织', '整改状态', '验证状态', '操作'];
    return `<div id="groupFocusObjectsSection"><div class="card-title" style="font-size:14px;padding:0 0 8px;border:0">重点监管对象</div>
    <div style="overflow-x:auto"><table class="data-table demo-focus-table"><thead><tr>${headers.map(h => `<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.map(r => `<tr><td><b>${r[0]}</b></td><td><span class="badge badge-info">${r[1]}</span></td><td>${r[2]}</td><td>${r[3]}</td><td>${r[4]}</td><td><span class="badge ${r[5].includes('红') ? 'badge-danger' : r[5].includes('橙') ? 'badge-warning' : 'badge-info'}">${r[5]}</span></td><td>${r[6]}</td><td>${r[7]}</td><td>${r[8]}</td><td><button class="btn btn-primary" style="padding:4px 12px;font-size:12px;white-space:nowrap" onclick="${r[9]}">向下穿透</button></td></tr>`).join('')}</tbody></table></div></div>`;
  },

  renderGroupResponsibilityClosureTrace() {
    return '';
  },
  ...
  renderDashboardGroupRegulatoryPerspectiveCard() {
    ...
    return `<div class="card" style="margin-bottom:20px">
      <div class="card-title">集团投资风险态势</div>
